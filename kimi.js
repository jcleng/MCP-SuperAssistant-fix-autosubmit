// ==UserScript==
// @name         kimi Auto Execute and Submit
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  自动获取call-id并点击执行按钮，然后将结果填入输入框提交
// @author       You
...
(function () {
     ...
     */
    function mainLoop() {
        // 1. 获取 .segment-code-content 的最后一个元素
        const segmentContents = document.querySelectorAll('.segment-code-content');
        if (segmentContents.length === 0) {
            console.log('[AutoScript] 未找到 .segment-code-content 元素');
            return;
        }

        const lastSegment = segmentContents[segmentContents.length - 1];
